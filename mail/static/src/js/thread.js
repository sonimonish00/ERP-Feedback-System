...
var ImageEditor = require('mail.ImageEditor');
var PDFEditor = require('mail.PDFEditor');
...
var Thread = Widget.extend({
    ...
    events: {
        ...
        "click .o_pdf_editor": 'open_pdf_editor',
    ...
    init: function (parent, options) {
        ...
        if(loc.indexOf('&image_id=') != -1){
            var fromurl = "&image_id=";
            var att_id = parseInt(loc.substring(loc.search(fromurl) + (fromurl).length));
            if (isNaN(att_id) == false) {
                var ir_attachment_model = new Model('ir.attachment');
                var domain = [['id', '=', att_id]];
                ir_attachment_model.call('search_read', [domain]).then(function(result) {
                    console.log(result);
                    if (result.length != 0) {
                        var image_editor = new ImageEditor(this, att_id,result[0].name, "image");
        ...
        if(loc.indexOf('&pdf_id=') != -1){
            ...
            if (isNaN(att_id) == false) {
                ...
                ir_attachment_model.call('search_read', [domain]).then(function(result) {
                    console.log(result);
                    if (result.length != 0) {
                        var pdf_editor = new PDFEditor(this, att_id,result[0].name, "pdf");
                        pdf_editor.appendTo('body');
                    }
                });
            }
        }
    ...
    open_image_editor: function(e){
        ...
        var att_name = $(e.currentTarget).data('att_name');

        var image_editor = new ImageEditor(this, att_id,att_name,"image");
        image_editor.appendTo('body');
    },
    open_pdf_editor:function(e){
        var att_id = $(e.currentTarget).data('att_id');
        var att_name = $(e.currentTarget).data('att_name');

        var pdf_editor = new PDFEditor(this, att_id,att_name,"pdf");
        pdf_editor.appendTo('body');
    }
});
